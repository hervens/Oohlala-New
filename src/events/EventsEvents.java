...
public class EventsEvents extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.eventsevents);
		
		currentView = true;
		
		TextView header = (TextView) findViewById(R.id.header);
		header.setTypeface(Fonts.getOpenSansBold(EventsEvents.this));
		...
		*/
		...
		adapter = new EventsEventsArrayAdapter(EventsEvents.this, list);
		    	...
		    	// TODO Auto-generated method stub
			   	if(scrollState == 0){
			   		if (last >= number && !lastcut){
			    		...
			    		cut++;
			    		TaskQueueImage.addTask(new getEventThread(), EventsEvents.this);
			    	} 
			    }
			}
		});
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			holder.tvTime.setVisibility(View.GONE);
				...
				if (list.get(position).user_attend == 1){
					...
					holder.bAttending.setText(getString(R.string._Attending));
				} else {
					//holder.bAttending.setText(list.get(position).attends + getString(R.string._Attending));
					holder.bAttending.setText(getString(R.string._Attending));
				}
				...
				holder.startTime.setTypeface(Fonts.getOpenSansLightItalic(EventsEvents.this));
				...
				holder.bg.setOnClickListener(new Button.OnClickListener() {
					public void onClick(View v) {
						...
						Intent i = new Intent(v.getContext(), EventsEventsDetail.class);
						i.putExtras(extras);
						startActivity(i);
					}
			    }); 
				...
				if (Integer.valueOf(list.get(position).start_time) < epoch_end_day && Integer.valueOf(list.get(position).end_time) > epoch_start_day && position == 0){
					...
					holder.tvTime.setText(getString(R.string.Today).toUpperCase());
				}
				...
				if (Integer.valueOf(list.get(position).start_time) >= epoch_end_day && 
					...
					//Log.i("coming soon", "-----------------");
					holder.tvTime.setVisibility(View.VISIBLE);
					holder.tvTime.setText(getString(R.string.Coming_Soon).toUpperCase());
				}
	    	...
	    	for (int i = start; i < stop; i++){
	    		if (i < list.size() && currentView){
		    		if (list.get(i).image_thumb != null && list.get(i).image_thumb.contains(".png") && list.get(i).store_bitmap == null){
						...
						list.get(i).store_bitmap = ImageLoader.studentsNearbyImageStoreAndLoad(list.get(i).image, EventsEvents.this, 50);
						mHandler.post(new Runnable() {
			        		public void run() {
						    	listView.invalidateViews();
						    }
						...
						//Log.i("image", list.get(i).image_url);
						list.get(i).store_bitmap = ImageLoader.studentsNearbyImageStoreAndLoad(list.get(i).store_logo, EventsEvents.this, 50);
						mHandler.post(new Runnable() {
			        		public void run() {
						    	listView.invalidateViews();
						    }
	    ...
	    public void run() {
	    	...
	    	for (int i = start; i < stop; i++){
	    		if (i < list.size()){
		    		if (list.get(i).image_thumb != null && list.get(i).image_thumb.contains(".png") && list.get(i).store_bitmap == null){
						//Log.i("image", list.get(i).image_url);
						list.get(i).store_bitmap = ImageLoader.studentsNearbyImageStoreAndLoad(list.get(i).image_thumb, EventsEvents.this, 50);
						mHandler.post(new Runnable() {
			        		public void run() {
						    	listView.invalidateViews();
						    }
